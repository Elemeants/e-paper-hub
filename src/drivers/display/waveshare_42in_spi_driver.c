...
__handle_data_command_pre_transmision(spi_transaction_t* trans) {
...
static void _ws42_driver_init_spi_device(void) {
  ...
  ret = spi_bus_initialize(driver_config.spi_bus, &buscfg, SPI_DMA_CH_AUTO);
  ESP_ERROR_CHECK(ret);
...
void ws42_driver_send_data_buffer(BYTE* data, WORD data_length) {
  ...
  memset(&t, 0, sizeof(t));
  t.length = BYTE_BITS * data_length;
  t.tx_buffer = data;
  t.user = (void*)1;
  error = spi_device_polling_transmit(screen_spi_handler, &t);
  assert(error == ESP_OK);
